...
defmodule Zoi.Types.Keyword do
  ...
  defimpl Zoi.Type do
        ...
        case map_fetch(input, key, coerce) do
          :error ->
            cond do
              ...
              default?(type) ->
                ...
                {Keyword.put(parsed, key, type.value), errors, path}
          ...
          {:ok, value} ->
            case do_parse(type, value, opts, path ++ [key], errors) do
              {:ok, val} ->
                {Keyword.put(parsed, key, val), errors, path}
              ...
              {obj_parsed, obj_errors, _path} ->
                {Keyword.put(parsed, key, obj_parsed), obj_errors, path}
            end
        end
      end)
    ...
    defp map_fetch(input_map, key, true = _coerce) do
      ...
      |> Keyword.fetch(to_string(key))
    end

    defp map_fetch(input_map, key, _coerce) do
      Keyword.fetch(input_map, key)
    end
  end
end
